...
2026-08-18、公開時の `main` を対象に3エージェントと機械ゲートで再監査し、教材・学習ループ・保存復帰・模試エンジンの実装ゲートは **SUCCESS** になりました。数学I・A、II・B・C、数学IIIを外部教材なしで進めるための320概念・320ガイド・320レッスン・2,392問と、9つの100点模試を収録しています。
...
- 初回のおすすめ開始、基礎スキップ後の共テ幹線自動遷移、模試優先のインポート復元、別タブ同期を実装しました。
- transferとdelayedをquickの単純な再表示にせず、別の見分け方・最初の一手を問う問題として監査します。
- 数学I・A、II・B・C、数学IIIを各3フォーム、100点・時間制限・誘導・未回答・中断復帰つきで収録しました。
- 模試結果に「提出前の解説非閲覧」フラグを保存し、G5の記録欠落を合格扱いしないようにしました。
- 数学IIIを教材スコープから除外していません。

## 現在の規模

- 全概念：320
- 共テ幹線概念：239（依存を含む主導線260、橋渡し21）
